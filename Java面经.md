...
- 效率问题：标记 清除效率都不高
- 空间问题：标记清除后悔产生大量不连续的内存碎片，不利于较大对象的分配

**复制算法**：内存按照容量划分为大小相等的两块，每次使用其中的一块，这一块使用完了，就将还存活的对象复制到另一块上，然后一次清理掉所有内存。

优点：不用考虑内存碎片，只需要移动栈顶指针

缺点：可用内存缩小为原来的一半

**标记-压缩算法**：标记过程与标记、清除算法相同，后续是上所有存活对象都向一端移动，然后直接清理边界以外的内存

**分代收集：**GC中有一个假设：绝大部分对象的生命周期都非常短暂。分代收集将Java分为新生代和老年代，根据各个年代的特点选择最适当的收集算法
...
新生代 老年代都采用串行回收；新生代复制，老年代标记-压缩；Stop the world
...
Parallel收集器更关注系统的吞吐量，可以通过参数打开自适应调节策略

Parallel Old收集器

是Parrallel的老年代版本，多线程，标记-整理算法
...
初始标记，重新标记需要暂停其他服务，初始标记仅仅只是标记一下GC Roots能直接关联到的对象，速度很快，并发标记阶段就是进行GC Roots Tracing的过程
...
1、空间整合：标记-整理算法，不会产生内存碎片，分配大对象时不会触发新一轮的GC

2、可预测停顿

G1中，Java堆内存布局与其他收集器有很大差别，它将J
=======
...
>>>>>>> 542d5bf5dfb481a335972883581acf61c492c510
